  ...
  return (
    <article className="max-w-content mx-auto">
      <div className="flex items-center gap-2 text-sm text-[rgba(255,255,255,0.6)] mb-6 justify-center">
        ...
        <span>/</span>
        <span className="font-bold text-black dark:text-white">Smart Money Concept</span>
      </div>
      
      <div className="text-center mb-8">
        <h1 className="text-3xl md:text-2xl lg:text-3xl font-bold leading-tight mb-6 tracking-tight line-clamp-3 break-words overflow-wrap-anywhere">{postData.title}</h1>
        
        <div className="flex items-center justify-center gap-2 text-[rgba(255,255,255,0.8)] text-sm mb-6">
          <div className="w-6 h-6 rounded-full bg-gray-600"></div>
          <span>{postData.author}{postData.authorRole && ` / ${postData.authorRole}`}</span>
        </div>
      </div>
